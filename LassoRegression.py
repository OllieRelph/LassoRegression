...
def lasso_regression(y_data, variance):
    x_data = []
    for x in range(1000):
        x_data.append(x)
    ...
    y_prediction = lassoreg.predict(x_test.reshape(-1,1))
    ...
    plt.ylabel("Output")
    plt.xlabel("Input")
    plt.title("Lasso Regression, Rnd Y Change = %s" %variance)
    plt.show()
    print("Mean Squared Error = %.2f" %MSE)
    print("R2 Score = %.2f" %r2score)


lasso_regression(create_y_data(1), "1")

lasso_regression(create_y_data(10), "10")

lasso_regression(create_y_data(50), "50")

lasso_regression(create_y_data(100), "100")